...
class ElMap_Perturbation_Analysis(object):
    ...
    def __init__(self, traj, spatial=1.0, shape=1.0, tangent=1.0, stretch=0.01, bend=0.001):
        self.tgt_traj = traj
        ...
        if self.n_dims == 1:
            ...
            exit()
        self.spatial_const = spatial
        self.tangent_const = tangent
        self.shape_const = shape
    ...
    def setup_problem(self):
        ...
        self.objective = cp.Minimize(self.spatial_const   * cp.sum_squares(I @ self.x - K @ self.traj_stacked) 
                                     + self.tangent_const * cp.sum_squares(E @ self.x - E @ self.traj_stacked) 
                                     + self.shape_const   * cp.sum_squares(R @ self.x - R @ self.traj_stacked) 
    ...
    def solve_problem(self, constraints, disp=True):
        ...
        self.problem.solve(verbose=disp, solver=cp.OSQP)
...
if __name__ == '__main__':
    ...
    PA = ElMap_Perturbation_Analysis(x_demo, spatial=1.0, shape=10000.0, stretch=5.0, bend=2.0)
    x_prob = PA.setup_problem()
    initial_freedom = 0.0
    endpoint_freedom = 0.0
    constraints = [cp.abs(x_prob[0] - x_demo[0] + 1) - initial_freedom <= 0, cp.abs(x_prob[-1] - x_demo[-1] + 1) - endpoint_freedom <= 0]
    sol_both = PA.solve_problem(constraints)
    
    PA = ElMap_Perturbation_Analysis(x_demo, spatial=1.0, shape=0.0, stretch=5.0, bend=2.0)
    x_prob = PA.setup_problem()
    initial_freedom = 0.0
    endpoint_freedom = 0.0
    constraints = [cp.abs(x_prob[0] - x_demo[0] + 1) - initial_freedom <= 0, cp.abs(x_prob[-1] - x_demo[-1] + 1) - endpoint_freedom <= 0]
    sol_spatial = PA.solve_problem(constraints)
    
    PA = ElMap_Perturbation_Analysis(x_demo, spatial=0.0, shape=10000.0, stretch=5.0, bend=2.0)
    x_prob = PA.setup_problem()
    initial_freedom = 0.0
    endpoint_freedom = 0.0
    constraints = [cp.abs(x_prob[0] - x_demo[0] + 1) - initial_freedom <= 0, cp.abs(x_prob[-1] - x_demo[-1] + 1) - endpoint_freedom <= 0]
    sol_shape = PA.solve_problem(constraints)
    
    plt.figure()
    plt.plot(x_demo, 'k', lw=5, label="Demo")
    plt.plot(sol_spatial, 'r', lw=5, label="Spatial Only")
    plt.plot(sol_shape, 'g', lw=5, label="Shape Only")
    plt.plot(sol_both, 'b', lw=5, label="Both")
    plt.legend()
    plt.show()
